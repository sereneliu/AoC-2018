...
class Unit:
    def __init__(self, species, location, points):
        self.species = species
        self.location = location
        self.points = points

def setup_combat(starting_pos):
    field = {}
    elves = []
    goblins = []
    for y in range(len(starting_pos)):
        for x in range(len(starting_pos[y])):
            if starting_pos[y][x] in ('#', '.'):
                field[(x, y)] = starting_pos[y][x]
            if starting_pos[y][x] == 'E':
                elves.append(Unit('elf', (x, y), 200))
            if starting_pos[y][x] == 'G':
                goblins.append(Unit('goblin', (x, y), 200))
    return field, elves, goblins

def sort_group(group):
    group.sort(key=lambda g: (g.location[1], g.location[0]))
    return group

def find_closest_enemies(unit, enemies):
    enemy_dist = {}
    min_dist = sys.maxint
    closest_enemies = []
    for enemy in enemies:
        min_dist = min(min_dist, man_dist(unit.location, enemy.location))
        enemy_dist[enemy.location] = man_dist(unit.location, enemy.location)
    for k, v in enemy_dist.items():
        if v == min_dist:
            closest_enemies.append(k)
    closest_enemies.sort(key=lambda e: (e[1], e[0]))
    return closest_enemies

def man_dist(unit, enemy):
    (a, b) = unit
    (c, d) = enemy
    return abs(a - c) + abs(b - d)

def move_unit(unit, enemy):
    up = man_dist(unit + (0, -1), enemy)
    left = man_dist(unit + (-1, 0), enemy)
    right = man_dist(unit + (1, 0), enemy)
    down = man_dist(unit + (0, 1), enemy)
    return min(up, left, right, down)

def combat(starting_pos):
    field, elves, goblins = setup_combat(starting_pos)
    units = elves + goblins
    units = sort_group(units)
    for unit in units:
        if unit.species == 'elf':
            enemies = find_closest_enemies(unit, goblins)
        else:
            enemies = find_closest_enemies(unit, elves)
        print move_unit(unit.location, enemies[0])
